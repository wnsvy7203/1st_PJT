        ...
        <table>
            ...
            <tbody>
              <tr v-for="reserved_meeting in reservedHistory" :key="reserved_meeting.meeting_id">
                ...
                <td><button class="delete-teacher-btn" @click="deleteMeeting(reserved_meeting.meeting_id)">삭제</button></td>
              </tr>
            </tbody>
          </table>
          <h2>최근 상담 내역</h2>
...
export default {
  ...
  setup() {
    ...
    const getCounselData = () => {
      ...
      .then((response) => {
        for (let i of response.data.teacherMasterMeetingList) {
    ...
    function deleteMeeting(id) {
      if (confirm('정말 삭제하시겠습니까?')) {
        axios({
          method: 'delete',
          url: `${store.state.API_URL}/api/v1/meetings/${id}`,
          headers: {
            Authorization: `Bearer ${store.state.token}`
          }
        })
