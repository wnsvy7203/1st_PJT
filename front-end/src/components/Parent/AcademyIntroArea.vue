...
export default {
   ...
   setup() {
    ...
    const register = () => {
      axios({
        method:'get',
        url: `${store.state.API_URL}/api/v1/parents/students/${store.state.student_id}/academies/${academy_id}`,
        headers: {
          Authorization: `Bearer ${store.state.token}`,
        },
      })
      .then(() => {
        ...
        router.push({ name : 'AcademyClassList', params:{student_id: store.state.student_id}})
      })
      .catch(() => {
        alert('이미 등록된 학원이거나, 신청 완료된 학원입니다.')
      })
    }
    onBeforeMount(() => {
      getInfo()
