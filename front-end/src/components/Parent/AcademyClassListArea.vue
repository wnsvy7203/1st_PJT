<template>
  <div class="container pt-100 pb-145">
    <div class="class-academy-container">
      <h2>{{ name }}의 반 목록</h2>
      <router-link to="/SearchAcademy"
        ><button class="btn"><span>학원등록</span></button></router-link
      >
      <hr />
      <div class="container">
        ...
        <div v-if="classesList.length < 1">등록된 학원이 없습니다</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const getAcademy = () => {
      axios({
        method: "get",
        url: `${store.state.API_URL}/api/v1/parents/students/${store.state.student_id}/classes`,
        headers: {
          Authorization: `Bearer ${store.state.token}`,
        },
      }).then((res) => {
